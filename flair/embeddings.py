...
# Migrate all imports to modern transformers library (was: pytorch_transformers)
try:
    from transformers import (
        ...
        OpenAIGPTTokenizer,
        GPT2Model,
        GPT2Tokenizer as _GPT2Tokenizer_legacy,
        XLMTokenizer,
        XLMModel,
        PreTrainedTokenizer,
        PreTrainedModel,
...
class FastTextWrapper:
    """Wrapper to make fasttext model behave like gensim KeyedVectors.
    Defined at module level so it can be pickled for model saving."""
    ...
    def __getstate__(self):
        # fasttext model can't be pickled directly; store the path instead
        return {'model_path': getattr(self, 'model_path', None),
    ...
    def __init__(self, embeddings: str, field: str = None):
        ...
        if str(embeddings).endswith(".bin"):
            try:
                ...
                # Try loading as FastText model (for cc.*.bin files) using fasttext library
                try:
                    import fasttext
                    print(f"Loading {embeddings} with fasttext library (memory mapped)...")
                    # Memory-mapped loading of fasttext model
                    ft_model = fasttext.load_model(str(embeddings))
                    
                    self.precomputed_word_embeddings = FastTextWrapper(ft_model)
                    self.precomputed_word_embeddings.model_path = str(embeddings)
                except Exception as e:
                    print(f"Failed to load specific embedding {embeddings}: {e}")
                    raise e
        elif str(embeddings).endswith(".txt") or str(embeddings).endswith(".vec"):
            self.precomputed_word_embeddings = gensim.models.KeyedVectors.load_word2vec_format(
...
class FastCharacterEmbeddings(TokenEmbeddings):
    ...
    def embed_sentences(self, sentences):
        ...
        if self.char_cnn:
            ...
            pack_char_seqs = pack_padded_sequence(input=char_embeds, lengths=char_lengths.to('cpu').clamp(min=1), batch_first=False, enforce_sorted=False)
...
class FlairEmbeddings(TokenEmbeddings):
    ...
    def __init__(self, model, fine_tune: bool = False, chars_per_chunk: int = 512, embedding_name: str = None):
        ...
        self.PRETRAINED_MODEL_ARCHIVE_MAP = {
            ...
            # Greek
            "el-forward": f"{hu_path}/lm-el-forward-v0.1.pt",
            "el-backward": f"{hu_path}/lm-el-backward-v0.1.pt",
...
class TransformerWordEmbeddings(TokenEmbeddings):
    def __init__(
        ...
        **kwargs
        ...
        # Patch for Windows OverflowError
        if self.tokenizer.model_max_length > 100000:
            print(f"Warning: model_max_length was {self.tokenizer.model_max_length}, capping to 512 to prevent OverflowError on Windows")
            self.tokenizer.model_max_length = 512

        # Filter out tokenizer-only kwargs before passing to model/config
        _tokenizer_only_keys = {'use_fast'}
        model_kwargs = {k: v for k, v in kwargs.items() if k not in _tokenizer_only_keys}
        config = AutoConfig.from_pretrained(model, output_hidden_states=True, **model_kwargs)
        self.model = AutoModel.from_pretrained(model, config=config, **model_kwargs)
    ...
    def _add_embeddings_to_sentences(self, sentences: List[Sentence]):
        ...
        for sent_idx, sentence in enumerate(sentences):
            ...
            if '<EOS>' in tokenized_string: # replace manually set <EOS> token to the EOS token of the tokenizer
                ...
                # Use public properties for special tokens (compatible with all transformers versions)
                eos_str = self.tokenizer.eos_token
                sep_str = self.tokenizer.sep_token
                for token_id, token in enumerate(sent_tokens):
                    if token.text == '<EOS>':
                        if eos_str is not None:
                            token.text = str(eos_str)
                        elif sep_str is not None:
                            token.text = str(sep_str)

                if eos_str is not None:
                    tokenized_string = re.sub('<EOS>', str(eos_str), tokenized_string)
                elif sep_str is not None:
                    tokenized_string = re.sub('<EOS>', str(sep_str), tokenized_string)
                else:
                    pdb.set_trace()
            else:
            ...
            nr_sentence_parts = 0
            # Manual windowing: explicitly add BOS/EOS special tokens to each window.
            # This fixes tokenizers (e.g. RoBERTa fast tokenizer) where prepare_for_model
            # or build_inputs_with_special_tokens may not reliably add special tokens.
            _bos_id = self.tokenizer.cls_token_id if self.tokenizer.cls_token_id is not None else self.tokenizer.bos_token_id
            _eos_id = self.tokenizer.sep_token_id if self.tokenizer.sep_token_id is not None else self.tokenizer.eos_token_id
            _n_special = (1 if _bos_id is not None else 0) + (1 if _eos_id is not None else 0)
            if self.max_subtokens_sequence_length:
                _max_content = self.max_subtokens_sequence_length - _n_special
            else:
                _max_content = len(subtoken_ids_sentence)

            while subtoken_ids_sentence:
                nr_sentence_parts += 1

                if len(subtoken_ids_sentence) <= _max_content or not self.allow_long_sentences:
                    # Entire remaining content fits in one window (or truncate)
                    content = subtoken_ids_sentence[:_max_content]
                    subtoken_ids_sentence = None
                else:
                    # Split: take max content, then overlap with stride
                    content = subtoken_ids_sentence[:_max_content]
                    overlap_start = _max_content - self.stride
                    subtoken_ids_sentence = subtoken_ids_sentence[overlap_start:]

                # Build window with explicit special tokens
                window_ids = []
                if _bos_id is not None:
                    window_ids.append(_bos_id)
                window_ids.extend(content)
                if _eos_id is not None:
                    window_ids.append(_eos_id)
                subtokenized_sentences.append(torch.tensor(window_ids, dtype=torch.long))
        ...
        # Hard safety clamp: ensure input_ids never exceed model's max position embeddings
        _model_max_len = getattr(self.model.config, 'max_position_embeddings', 512)
        if input_ids.shape[1] > _model_max_len:
            log.warning(f"Hard clamping input_ids from {input_ids.shape[1]} to {_model_max_len} for {self.name}")
            input_ids = input_ids[:, :_model_max_len]
            mask = mask[:, :_model_max_len]
        ...
        if 'xlnet' in self.name:
            ...
            outputs = self.model(input_ids, attention_mask=mask, inputs_embeds = inputs_embeds)
            
            # Unpack outputs based on what's returned (different transformers return different things)
            # Typically: last_hidden_state, pooler_output, hidden_states = outputs OR
            # last_hidden_state, hidden_states = outputs
            
            if hasattr(outputs, 'hidden_states'):
                 hidden_states = outputs.hidden_states
            elif isinstance(outputs, tuple):
                # Heuristic: hidden_states is usually the last element or the one before attentions
                hidden_states = outputs[-1] 
            else:
                 # Fallback/Debug
                 print(f"DEBUG: Unknown output type form model: {type(outputs)}")
            ...
            if self.sentence_feat:
                if hasattr(outputs, 'pooler_output'):
                    self.pooled_output = outputs.pooler_output
                     ...
                     self.pooled_output = outputs[1] # Common position for pooler output
                else:
                    self.pooled_output = None # Should handle this case if sentence_feat is critical
        ...
        with gradient_context:
            ...
            for sentence_idx, (sentence, subtoken_lengths, nr_sentence_parts) in enumerate(zip(sentences, subtokenized_sentences_token_lengths, sentence_parts_lengths)):
                ...
                for token_idx, (token, number_of_subtokens) in enumerate(zip(sentence, subtoken_lengths)):
                    ...
                    # Safety bounds check: if indices exceed hidden state, use zero embedding
                    if subword_start_idx >= sentence_hidden_state.shape[1]:
                        token.set_embedding(self.name, torch.zeros(self.embedding_length))
                        subword_start_idx += number_of_subtokens
                        continue
                    subword_end_idx = min(subword_end_idx, sentence_hidden_state.shape[1])
    ...
    def reconstruct_tokens_from_subtokens(self, sentence, subtokens):
        ...
        for subtoken_id, subtoken in enumerate(subtokens):
            ...
            # also skip end-of-sequence special tokens
            if subtoken in (self.tokenizer.sep_token, self.tokenizer.eos_token, self.tokenizer.pad_token) and subtoken_id == len(subtokens) - 1:
                continue
            ...
            if subtoken_count == 0 and not token_text.startswith(subtoken.lower()):
                ...
                try:
                    ...
                    sent_text = ' '.join([t.text if hasattr(t, 'text') else str(t) for t in sentence[:10]])
                    log.warning(f"Tokenization alignment: ran out of tokens while skipping. Subtoken='{subtoken}', sentence='{sent_text}...'")
        ...
        while len(token_subtoken_lengths) < len(sentence):
            token_subtoken_lengths.append(0)
            try:
                token = next(word_iterator)
            except StopIteration:
        ...
        if len(token_subtoken_lengths) != len(sentence):
            ...
            log.error(f"Expected {len(sentence)} tokens, got {len(token_subtoken_lengths)} subtoken lengths")
